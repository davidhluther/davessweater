...
- [x] **leaf-v1 calibration — DONE 2026-09-04** (branch `leaf-v1-calibration-2026-09-04`,
      pushed, no PR: a change to a graded season wants a review pass). Owner ruling (David):
      the regional forecaster's past data and observations may be used to TRAIN the model —
      facts are ingestable, prose is not, and nothing user-facing cites him. Supersedes the
      "revisit after Nov 10" framing of the two items below, which were both answerable from
      18 years of published record without waiting for our own graded season.
      - **Training set: `data/leaf/training/fcg-historical.json`** — 18 years on record
        (2008–2025), 17 with an observed peak (2024 excluded for Helene), each with the
        September mean published alongside it. The per-year values are published as charts,
        not tables; every pair was digitized and cross-validated across three independent
        figures in the same document, and the assembled set reproduces the source's own
        stated regression (1.9015, R² 0.62) to three decimals. No prose is stored.
      - **Cross-check, our thermal instrument vs theirs:** r = 0.80, mean offset −0.53 °F,
        mean |Δ| 1.11 °F, worst year 2.54 °F. Too loose to borrow their temperatures — so v1
        pairs **our Open-Meteo series with their observed peaks**, which also fits better
        (R² 0.673 vs 0.617).
      - **Fitted coefficient 1.80 days/°F** on the full September mean (1.65 on the Sep 1–25
        provisional window). LOYO MAE 2.34 days vs 3.85 for climatology-only; the spread
        between 1.5 / 1.8 / 1.9 is 0.16 days, inside the ground truth's own ±3-day
        resolution. Sensitivity holds as warned: drop 2018–2019 and R² falls 0.673 → 0.354.
      - **Clamp widened ±7 → ±10.** 2019 implies +8.6 days and observed +12.5; a ±7 clamp
        makes the largest documented miss larger on purpose.
      - **Anchor unchanged at October 6** — and now corroborated: 17 observed peaks average
        October 16.5 at 3,300 ft, implying an October 5.5 anchor at 5,000 ft (October 6.8 for
        2017+).
      - **Climatological normal changed to a fixed 2008–2025 span.** v0's 6-year rolling
        window sits 1.30 °F cooler than the long-term mean at Boone, which the fitted
        coefficient would have multiplied into ~2.3 days of spurious delay every year.
      - **Drought/precip recorded as a TESTED EXCLUSION** in `docs/leaf-model.md`, not an
        omission. No term added.
      - **Integrity:** `data/leaf/predictions.json` (the July 26 v0-draft artifact) is
        untouched and is no longer regenerated by any workflow. `predict_leaf.py` now refuses
        to overwrite any existing prediction file without `--allow-overwrite`.
      - **Two-pass refresh** in `daily_capture.yml`: Sep 18–30 writes the v1 provisional pass,
        Oct 6–12 writes the v1 final pass on the full September mean. `score_leaf.py` grades
        every prediction file into its own scoreboard (`scores.json`, `scores-v1-provisional.json`,
        `scores-v1.json`) — never merged. `src/lib/leaf.ts` reads the newest file that exists.
      - Reproduce every number with `python3 scripts/fit_leaf_v1.py`. Honest-limits section
        added to `docs/leaf-model.md` (n≈18 and effectively n≈2; one elevation, one observer;
        ground truth coarser than the fit).
...
- [x] **Independent review + re-review, then MERGED to main — 2026-09-04.** Reviewed by an
      agent that did not build the branch, in a throwaway worktree, per the branch's own
      "a change to a graded season wants a review pass".
      - **Every documented number reproduced exactly** from `scripts/fit_leaf_v1.py`:
        1.798 / 1.650 / 1.902 slopes, R² 0.673 → 0.354 without 2018–19, LOYO 2.34 vs 3.85
        climatology, the three clamp rows, and the 1.30 °F / ~2.3-day normal-shift effect.
      - **Training-set faithfulness is stronger than the doc claims.** The published-series
        regression reproduces at slope 1.90151 / intercept −106.4200 — four decimals, not
        three. Two further statistics *not used in the fit* also reproduce exactly from the
        digitized dates: era variances 9.0 (2008–16) and 38.1 (2017–25), and the long-term
        mean 64.711 vs the published 64.7 with min/max landing on the stated 62.1 (2012) /
        69.4 (2018). Independent corroboration that the chart digitization is faithful.
      - **Freeze verified by attack, not by reading.** `data/leaf/predictions.json` stayed
        byte-identical (md5 `ffef853d…`) through a `--model-version leaf-v0-draft` run, an
        explicit `--out data/leaf/predictions.json` attempt (the guard is on the *resolved*
        path, so `--out` cannot bypass it), and a real provisional run. `score_leaf.py`
        wrote separate scoreboards and its year gate refused the four 2025 backfill rows.
      - **Re-review of the fix:** `/leaf` renders truthful prose for both versions —
        v0 "6-year normal / 1.5 days per degree / 7 days / first guess", and via a v1 final
        fixture "18-year normal / 1.8 days per degree / 10 days / fitted, and thin". Both
        themes at 390 and 1440, no overflow; design gate mechanical PASS (0 sev-1, 0 sev-2,
        4 captures). Fixture removed, tree clean, frozen artifact still `ffef853d…`.
      - **Verified in-repo, not taken on report:** `pytest` 657 passed + 1 skipped (658
        collected, up from 642), `npm test` 382, `npm run build` green, `copy_lint` 0 errors
        with no new warnings. `tests/test_predict_leaf.py` spot-read — real assertions, not
        tautologies, covering exactly the untested paths the review flagged.
      - **One review sev-2 was withdrawn as a reviewer error:** "the overwrite guard exits 0"
        was a shell-pipeline measurement artifact (`$?` read `tail`, not python). `main()`
        returns 1 and `test_main_refuses_to_overwrite_existing_file` proves it.
      - **Open for the next session, not a defect:** the `/leaf` copy now says "6-year
        normal" and "18-year normal" without the rolling-vs-fixed distinction the doc makes.
        Both readings are true; the nuance lives in `docs/leaf-model.md`. Revisit only if a
        reader trips on it.
...
- [x] **Internal links to `/leaf` — DONE 2026-08-31 with the `/leaf` build.** All three
      staged slots landed exactly as specified: `SiteFooter.tsx` ("Leaf forecast", all 60
      URLs), the marked LINK SLOT in `FallColorWindow.tsx` (17 contextual in-body links),
      and a homepage line near `AlsoTracking` — that one is a component
      (`LeafSeasonPrompt.tsx`) rather than inline JSX so it can **self-retire**, rendering
      only while a predicted window is still current under the same 45-day grace the town
...
- [~] **OG/social cards for `/leaf` and `/tourism`.** House pattern is prerendered static
      ...
      **`/leaf` DONE 2026-08-31** — `ogPath.leaf` / `ogAlt.leaf` added, the card is built
...
- [ ] **Decide whether town-page metadata gets a seasonal fall-color variant.** Not done
      ...
      H2 plus the unique per-town data is doing the topical work for now.
      **/leaf-build call 2026-08-31: still no.** The hub now owns the seasonal query
      surface with its own title, description, FAQ schema and Dataset schema, so the case
      for churning 17 town titles during the ramp got weaker, not stronger. Revisit after
      the season with GSC data on what `/leaf` actually caught, which is a real answer
      instead of a guess. Owner can overrule.
...
- [~] **Leaf-season forecaster — ✅ DRAFT MODEL MERGED 2026-07-26, ~7 weeks ahead of target (PR #142;
      ...
      `FallColorWindow.tsx` (fall-readiness sweep, top of this file).
      **UPDATE 2026-08-31 — the product shipped; only traffic v2 and the live season remain.**
      - [x] **`/leaf` hub LIVE.** Static route, no dynamic segment, zero new functions (budget
            holds at 10 of 10; the build reports `○ /leaf`). Carries the cross-town gradient as an
            elevation-band strip drawn from the data, all 18 windows in a table that fits a 390px
            phone without sideways scroll, the full methodology with its constants read from the
            artifact, the model's stated limits, and the grading contract. WebPage + Dataset (CC BY
            4.0) + BreadcrumbList + FAQPage schema. Reuses `src/lib/leaf.ts` as instructed; the
            cross-town helpers were added there rather than re-parsing the JSON.
      - [x] **Follow-up (a) mid-Sept refresh: AUTOMATED, not left to memory.** `daily_capture.yml`
            gained a date-gated step running `predict_leaf.py --refresh` **Sep 18–30** (the model
            needs 12 September days and the Open-Meteo archive trails ~6, so Sep 18 is the earliest
            useful run; the anomaly window closes Sep 25, so running past Sep 30 would only churn
            published dates). Gate verified against seven boundary dates. `data/leaf/` was added to
            the workflow's `git add` set — without that the refresh would have run and been thrown
            away every morning.
      - [x] **Follow-up (b) October observations: format decided and shipped.**
            `data/leaf/observations.json` is the hand-maintained file, seeded empty with its schema,
            its weekly reading plan, and the three grading sources. An entry names a town **or** an
            elevation band (the published reports describe the season by band, so a band entry
            scores every town inside it) and must carry a `source_id`; a test fails the suite if one
            appears without a source. `scripts/score_leaf.py` joins observations to predictions using
            the July scoring rules — unchanged, so nothing is graded on a curve — and writes
            `data/leaf/scores.json`; it runs daily in the workflow. `/leaf` renders "nothing is
            scored yet" until `summary.scored_rows` goes above zero, then switches to the scoreboard
            on its own. **Weekly cadence: read all three sources every Monday, Sept 21 through Nov 9**
            (plan in `docs/leaf-model.md` §Grading in practice and in the observations file).
      - [x] **Busy-ness Index up-weight WIRED.** `compute_busyness.py` gained a 0–15 `leaf`
            component: the share of tracked towns whose predicted window covers the date, scaled to
            the cap. The registry's flat `leaf-season-2026` placeholder now carries
            `superseded_by: data/leaf/predictions.json` and the engine skips any season with that
            field, so the fact is never double-counted; a test guards the flag. The curve runs ~0.8
            pts in early October, peaks ~11.7 on Oct 19, and decays to ~1.7 by November, which is
