...
Last verified against repo: 2026-09-03 for the road-conditions row only (a Parkway-alerts coverage
caveat was added from the leaf benchmark's cross-check; no other row was re-checked that day).
Prior full pass: 2026-08-31 (leaf-forecaster build; rows touched: the `/leaf`
...
| Winter road-condition forecast plus live closures, incidents, and Parkway alerts | `scripts/roads.py`, `capture_roads.py`, `compare_roads.py`; `/roads` | 2026-07-25 | <https://davessweater.com/roads> (200); `data/roads_forecast.json`, `data/road_scores.json`; `tests/test_roads.py`. **Parkway alerts caveat, found 2026-09-03:** the NCDOT legs are live and populated, but `parkway_alerts` has been `[]` in every daily file from 2026-08-07 to 08-31 while NPS lists five closed NC segments. Cause not yet determined — a missing `NPS_API_KEY` and an endpoint that carries no closures produce byte-identical output, and `fetch_ok` covers only the DriveNC calls. Do not read an empty Parkway section as a quiet Parkway until the check on `CHECKLIST.md` is done |
