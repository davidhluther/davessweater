...
Last verified against repo: 2026-09-03 (tourism v1 build; rows added: the `/tourism`
Busy-ness Index hub, its vs-typical derivation, and the `/api/v1/tourism` + busyness-feed
endpoints. Rows not named in this or the preceding pass carry their earlier verification
date and were not re-checked.)

Prior pass, 2026-08-31 (leaf-forecaster build; rows touched: the `/leaf`
...
| Public Busy-ness Index: this weekend's crowding call with named drivers, a vs-typical percentile, the 14-day horizon, a 30-day lodging heat calendar, the weekend rate trend, and the full formula | `/tourism` (static route, no dynamic segment, zero functions) reading `data/demand/index/*.json`, `data/demand/*.json`, and `data/events/registry.json` through `src/lib/tourism.ts` | 2026-09-03 | Build reports `○ /tourism` (Static) and `check_function_budget.py` holds at 10 of 10. Inbound links from `SiteFooter` (all 61 URLs), `/roads`, and `/api`. Sitemap entry stamped from the artifact's own `computed_at` (`2026-09-02T14:17:08.373Z` in the built sitemap). Static share card at `public/og/tourism.png` |
| "Vs typical" ranking of a scored night against the whole index archive, controlling for day class and lead time | `src/lib/tourism.ts` (`vsTypical`, `comparablePool`, `percentileRank`) over every `(issued, target)` pair in `data/demand/index/` | 2026-09-03 | `src/lib/__tests__/tourism.test.ts` (37 tests). Live on ship-day data: Sat 2026-09-05 at 82 ranked above 92% of 54 comparable weekend nights (median 52). Returns **null**, and the page prints no percentage, below 12 comparable nights |
| Busy-ness Index published as JSON and RSS | `/api/v1/tourism` (a HANDLERS entry in the existing `/api/v1/[endpoint]` catch-all) and `/feed/high-country/busyness.xml` (a variant of the existing `/feed/[town]/[feed]` route) | 2026-09-03 | `src/app/api/__tests__/v1Route.test.ts` exercises both detail levels and the 400 against real committed data; the feed prerenders to `.next/server/app/feed/high-country/busyness.xml.body`. **Neither adds a function** — that was the whole point of routing them through existing route files |
