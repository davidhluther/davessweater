...
export default function SiteHeader({ towns }: { towns: TownNavItem[] }) {
  ...
  // The desktop "Towns" dropdown that used to live in this nav was replaced
  // 2026-07-28 by TownPicker, the single header-bar town control (it shows the
  // current town and is visible at every width). Two town controls in one bar
  // would compete; the mobile sheet keeps its collapsible Towns section because
  // it is a different surface — a menu you opened, not the bar itself.
  //
  ...
  // whitespace-nowrap: the desktop row is tight enough that "Right/Wrong Ray"
  // and "Swag Shop" were breaking mid-label into two-line pills. The nav's
  // natural single-line width fits the container; only the wrapping made it
  // look like it didn't.
  const pill = (active: boolean) =>
    cn("whitespace-nowrap rounded-md px-2.5 py-1.5 text-[0.78rem] font-medium transition-colors",
  ...
  return (
    <header className="sticky top-0 z-50 border-b-4 border-orange bg-teal-700">
      <div className="mx-auto flex w-full max-w-5xl items-center gap-2 px-4 py-2.5 md:gap-3">
        ...
        {/* The town control leads the right-hand cluster on desktop and sits
            beside the hamburger on mobile — one instance, both layouts. It
            carries the only ml-auto in the row, so everything after it packs
        ...
        <button type="button" aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open}
          onClick={() => setOpen((v) => !v)}
          className="-mr-1.5 inline-flex size-11 shrink-0 items-center justify-center rounded-md text-white md:hidden">
